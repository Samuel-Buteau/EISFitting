...
class DenseResBlock(tf.layers.Layer):
    def __init__(self, filters, dropout=0.2,
                 trainable=True, name=None, dtype=None,
                 activity_regularizer=None, **kwargs):
        ...
        self.dense1 = tf.layers.Dense(
            ...
            name="dense1", kernel_initializer=tf.initializers.orthogonal)
        self.dense2 = tf.layers.Dense(
            units=filters, activation=None,
            name="dense2", kernel_initializer=tf.initializers.orthogonal)
        self.down_sample = None

    def build(self, input_shape):

        channel_dim = 1
        self.dropout1 = tf.layers.Dropout(self.dropout, [tf.constant(1), tf.constant(self.filters)])
        self.dropout2 = tf.layers.Dropout(self.dropout, [tf.constant(1), tf.constant(self.filters)])
        if input_shape[channel_dim] != self.filters:
            self.down_sample = tf.layers.Dense(
                    units=self.filters, activation=None,
                    name="dense2", kernel_initializer=tf.initializers.orthogonal)
        self.built = True
...
class ParameterVAE(object):
    ...
    def __init__(self, kernel_size, conv_filters, dense_filters, num_conv, num_dense, trainable, num_encoded):
        ...
        for i in range(self.num_dense):
            if i == self.num_dense-1:
                filters = self.num_encoded
            else:
                filters = self.dense_filters
            self.dense_layers.append(
                DenseResBlock(filters=filters,
                              dropout=self.dropout, trainable=trainable,
                              name="dense_res_{}".format(i)))
    ...
    def build_forward(self, inputs, real_inputs, batch_size, priors, no_random_decode = True):
        ...
        single_vector= tf.reduce_sum(hidden_weights*hidden_values, axis=1, keepdims=False)

        hidden = single_vector
    ...
    def optimize_direct(self, inputs, prior_mu, prior_log_sigma_sq,
        ...
        #reconstruction_loss = tf.losses.mean_squared_error(impedances, inputs[:, :, 1:], weights=tf.reshape(tf.constant([1.,10.]),[1,1,2]) *tf.ones_like(impedances) )
...
def train_on_real_data(args):
    ...
    for s in sorted_sorted_results:
        groupby.record(len(s[2]), s)
    ...
    for k in groupby.data.keys():
        datasets.append(
            tf.data.Dataset.from_tensor_slices(
                (numpy.array([x[2] for x in groupby.data[k]]),
                 numpy.array([x[3] for x in groupby.data[k]]))).repeat())
        number_per_datasets.append(len(groupby.data[k]))

    choice_dataset = tf.data.Dataset.range(len(number_per_datasets)).repeat().shuffle(buffer_size=10)
    train_balanced_dataset = tf.data.experimental.choose_from_datasets(datasets, choice_dataset)
...
def run_on_real_data(args):
    ...
    model = ParameterVAE(kernel_size=args.kernel_size, conv_filters=args.conv_filters,
                         dense_filters=args.dense_filters, num_conv=args.num_conv,
                         num_dense=args.num_dense, trainable=False, num_encoded=number_of_params)
