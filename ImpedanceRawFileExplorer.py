...
filename = ".\\Jes-JEH_FRA_63473_Nw_c160_41V_40C_170306-030617-EIS0002.FRA"
                        ...
                        for index in range(len(turning_points)-1):
                            ...
                            if not len(log_freq_) < 10:
                                ...
                                if True:
                                    ...
                                    #test_0 = new_filename in bad_files
                                    if False:  #if (test_0 and not test) or (test and not test_0):
                                        print('mean mag_ {}, mean mag {}, mean dev {}'.format(mean_mag_, mean_mag, mean_dev))
                                        fig = plt.figure()
                                        ax = fig.add_subplot(1, 2, 1)
                                        ax.scatter(re_z, -im_z)

                                        if test:
                                            plt.title("failed tests: t1:{}, t2:{}, t3:{}".format(test_1, test_2, test_3))


                                        ax = fig.add_subplot(1, 2, 2)
                                        ax.scatter(re_z[1:]-re_z[:-1], -(im_z[1:] - im_z[:-1]))
                                        plt.show()
...
with open(os.path.join(".", "RealData", "database_eis.file"), 'wb') as f:
...
with open(os.path.join(".", "RealData", "database.file"), 'wb') as f:
